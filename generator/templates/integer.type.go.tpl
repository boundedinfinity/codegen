{{ define "integer_type" }}
    ...
    func (this {{ $tname }}) Validate() error {
        ...
        {{ template "integer_validation" dict "Name" $tname "Instance" . "Path" "this" }}
...
{{ define "integer_validation" }}
    {{ if eq .Instance.Schema "integer" }}
        {{ if .Instance.Min.Defined -}}errs = append(errs, validation.IntegerMin("{{ .Name }}", {{ .Instance.Min.Get }}, {{ .Path }})){{ end }}
        {{ if .Instance.Max.Defined -}}errs = append(errs, validation.IntegerMax("{{ .Name }}", {{ .Instance.Max.Get }}, {{ .Path }})){{ end }}
        {{ if .Instance.MultipleOf.Defined -}}errs = append(errs, validation.IntegerMultipleOf("{{ .Name }}", {{ .Instance.MultipleOf.Get }}, {{ .Path }})){{ end }}
        {{ if .Instance.Negative.Defined -}}errs = append(errs, validation.IntegerNegative("{{ .Name }}", {{ .Path }})){{ end }}
        {{ if .Instance.Negative.Defined -}}errs = append(errs, validation.IntegerNegative("{{ .Name }}", {{ .Path }})){{ end }}
    {{ end }}
{{ end }}
