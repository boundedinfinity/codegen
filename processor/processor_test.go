...
func assertString(tt *testing.T, proc *processor.Processor, expected *model.CodeGenString) {
	...
	if ok1 {
		assert.Equal(tt, expected.Schema(), typ.Schema())
		actual, ok2 := typ.(*model.CodeGenString)
		assert.True(tt, ok2)
...
func assertObject(tt *testing.T, proc *processor.Processor, expected *model.CodeGenObject) {
	...
	if ok1 {
		assert.Equal(tt, expected.Schema(), typ.Schema())
		actual, ok2 := typ.(*model.CodeGenObject)
		assert.True(tt, ok2)
		assert.Equal(
			tt, expected, actual,
			"expected : %s\nactual: %s", spew.Sdump(expected), spew.Sdump(actual),
		)
	}
}
